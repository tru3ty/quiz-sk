...
export default function ScheduleCard({ ev, onBook }) {
  ...
  return (
    ...
    >
      <div className="text-center font-[family-name:--font-unbounded]">
        <div className="text-2xl sm:text-[32px] font-black leading-none" style={{ color }}>
          {String(ev.date).padStart(2, "0")}
        </div>
        <div className="text-[10px] font-[family-name:--font-mono] text-[--color-orb-sub] uppercase tracking-[0.2em] mt-1">
          {ev.day} · {ev.time}
        </div>
      </div>

      <div className="min-w-0">
        <div className="text-base sm:text-lg font-bold font-[family-name:--font-unbounded] tracking-tight truncate">
          {ev.title}
        </div>
        <div className="flex gap-2 mt-1.5 items-center flex-wrap">
          <span className="text-xs text-[--color-orb-sub] font-[family-name:--font-mono]">▣ {ev.host}</span>
          {ev.tags.map((t) => (
            <span
              key={t}
              className="text-[10px] px-2 py-0.5 rounded-full border font-[family-name:--font-mono] uppercase tracking-[0.12em]"
